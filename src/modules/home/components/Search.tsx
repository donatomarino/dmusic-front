...
const Search = ({ auth }: VerifiedAuth) => {
  ...
  return (
  <div className="bg-bg-cards h-screen p-4 flex flex-col"> {/* h-screen para ejemplo */}
    ...
    {/* Contenedor scrollable: aquí están las claves */}
    <div className="w-full flex-1 min-h-0 overflow-y-auto p-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {search.map((e) => (
          <div
            className="bg-night rounded-2xl shadow-md transition-transform duration-200 hover:scale-105 hover:shadow-lg hover:bg-hover-night flex flex-col items-center p-3"
            key={e.id}
          >
            <img
